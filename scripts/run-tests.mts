...
// ─── Part 2: Coach Engine (child process) ───────────────────────────────────

console.log('Running Part 2: Coach Engine tests (subprocess)...');
try {
  const coachOut = execFileSync('npx', ['tsx', 'scripts/coach-runner.ts'], {
    cwd: ROOT,
    encoding: 'utf8',
    timeout: 30000,
  });
  const coachResults = JSON.parse(coachOut.split('\n').find((l: string) => l.startsWith('{')));
  PASS += coachResults.PASS;
  FAIL += coachResults.FAIL;
  for (const cr of coachResults.RESULTS) RESULTS.push(cr);
} catch(e: any) {
  console.error('Coach runner failed:', e.message);
  FAILURES.push({ testId:'COACH', id:'COACH:ERR', expected:'pass', actual: e.message });
  FAIL++;
}
