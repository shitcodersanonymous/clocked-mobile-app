...
// Dynamic resolve with tsconfig-paths
const { register } = await import('tsconfig-paths');
const tsconfig = JSON.parse(await (await import('fs/promises')).readFile(ROOT+'/tsconfig.json','utf8'));
const cleanup = register({ baseUrl: ROOT, paths: { '@/*': ['./*'] } });

const { generateCoachRecommendation, recommendationToPrompt } = await import(ROOT+'/lib/coachEngine.ts');
